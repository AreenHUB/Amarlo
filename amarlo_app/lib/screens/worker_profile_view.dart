...
class _WorkerProfileViewPageState extends State<WorkerProfileViewPage> {
  ...
  void _openReviewSheet() {
    ...
    ).then((submitted) { if (submitted == true) _load(); });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final avg = _reviews.isEmpty ? 0.0
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_conductWarnings >= 3) ...[
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                          child: Text(
                            'This user has received $_conductWarnings conduct reports.',
                  ...
                  // ── About ─────────────────────────────
                  if (_worker!.introduction?.isNotEmpty == true) ...[
                    const Text('About Me',
                  ...
                  // ── Social Media ──────────────────────
                  if (_SocialLinks.hasSocial(_worker!)) ...[
                    const Text('Social Media',
                        style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 10),
                    _SocialLinks(worker: _worker!),
                    ...
                    ..._services.map((s) => Card(
                          ...
                          child: ListTile(
                            leading: ClipRRect(
                              ...
                              child: AppNetworkImage(imageUrl: s.imageUrl, width: 50, height: 50),
                            ),
                            title:    Text(s.name),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _RatingRow('Quality',       r.qualityRating.toDouble()),
                                _RatingRow('Punctuality',   r.punctualityRating.toDouble()),
                                _RatingRow('Communication', r.communicationRating.toDouble()),
                                if (r.comment != null) ...[
                                  const SizedBox(height: 6),
                                  Text(r.comment!,
                                      style: const TextStyle(fontSize: 13, color: Colors.black87)),
                                ],
                                if (r.reviewerEmail == context.read<AuthProvider>().user?.email) ...[
                                  const SizedBox(height: 6),
                                  Align(
                                    ...
                                    child: TextButton.icon(
                                      onPressed: () async {
                                        ...
                                        if (ok == true) {
                                          await ApiService.deleteReview(r.id!);
                                          _load();
                                        }
                                      },
...
class _SocialLinks extends StatelessWidget {
  final User worker;
  const _SocialLinks({required this.worker});

  static bool hasSocial(User u) =>
      (u.facebook?.isNotEmpty  ?? false) ||
      (u.instagram?.isNotEmpty ?? false) ||
      (u.telegram?.isNotEmpty  ?? false) ||
      (u.linkedin?.isNotEmpty  ?? false);
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      runSpacing: 10,
      children: [
        if (worker.facebook?.isNotEmpty == true)
          _SocialButton(
            label:     'Facebook',
            icon:      FontAwesomeIcons.facebook,
            iconColor: const Color(0xFF1877F2),
            bgColor:   const Color(0xFFE8F0FE),
            handle:    worker.facebook!,
            onTap: () => _launch(_facebookUrl(worker.facebook!)),
          ),
        if (worker.instagram?.isNotEmpty == true)
          _SocialButton(
            label:     'Instagram',
            icon:      FontAwesomeIcons.instagram,
            iconColor: const Color(0xFFE1306C),
            bgColor:   const Color(0xFFFCE4EC),
            handle:    worker.instagram!,
            onTap: () => _launch(_instagramUrl(worker.instagram!)),
          ),
        if (worker.telegram?.isNotEmpty == true)
          _SocialButton(
            label:     'Telegram',
            icon:      FontAwesomeIcons.telegram,
            iconColor: const Color(0xFF0088CC),
            bgColor:   const Color(0xFFE3F2FD),
            handle:    worker.telegram!,
            onTap: () => _launch(_telegramUrl(worker.telegram!)),
          ),
        if (worker.linkedin?.isNotEmpty == true)
          _SocialButton(
            label:     'LinkedIn',
            icon:      FontAwesomeIcons.linkedin,
            iconColor: const Color(0xFF0A66C2),
            bgColor:   const Color(0xFFE8F4FD),
            handle:    worker.linkedin!,
            onTap: () => _launch(_linkedinUrl(worker.linkedin!)),
          ),
      ],
    );
  }
  ...
  // ── URL builders ────────────────────────────────────
  // Accept either a full URL or just a username/handle.

  static String _clean(String s) => s.trim().replaceAll('@', '');

  static String _facebookUrl(String v) {
    if (v.startsWith('http')) return v;
    return 'https://www.facebook.com/${_clean(v)}';
  }

  static String _instagramUrl(String v) {
    if (v.startsWith('http')) return v;
    return 'https://www.instagram.com/${_clean(v)}';
  }

  static String _telegramUrl(String v) {
    if (v.startsWith('http')) return v;
    // Prefer the deep-link so the app opens directly
    final handle = _clean(v);
    return 'https://t.me/$handle';
  }

  static String _linkedinUrl(String v) {
    if (v.startsWith('http')) return v;
    return 'https://www.linkedin.com/in/${_clean(v)}';
  }

  // ── Launch ───────────────────────────────────────────
  static Future<void> _launch(String url) async {
    final uri = Uri.parse(url);
    // Try to open in the native app first; fall back to browser.
...
// ─────────────────────────────────────────────────────────────────────────────
//  Single social button chip
// ─────────────────────────────────────────────────────────────────────────────
class _SocialButton extends StatelessWidget {
  final String     label;
  final FaIconData icon;   // FontAwesome icon — not Flutter's IconData
  ...
  @override
  Widget build(BuildContext context) {
    // Show the handle stripped of protocol/domain for readability.
    final display = _displayHandle(handle);
        ...
        child: Padding(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            FaIcon(FontAwesomeIcons.arrowUpRightFromSquare,
                size: 11, color: iconColor.withValues(alpha: 0.6)),
          ]),
        ),
      ),
    );
  ...
  /// Strip https://www.domain.com/ leaving just the handle/path.
  static String _displayHandle(String v) {
    if (!v.startsWith('http')) return v.replaceAll('@', '');
    try {
      final uri = Uri.parse(v);
      final path = uri.path.replaceAll('/', '').trim();
      return path.isNotEmpty ? path : v;
    } catch (_) {
      return v;
    }
  }
